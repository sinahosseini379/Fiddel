...
module("luci.controller.fiddel", package.seeall)
...
function index()
	if not nixio.fs.access("/etc/config/fiddel") then
		if nixio.fs.access("/usr/share/fiddel/0_default_config") then
			luci.sys.call('cp -f /usr/share/fiddel/0_default_config /etc/config/fiddel')
		else return end
	end
	local api = require "luci.fiddel.api"
	...
	--[[Components update]]
	entry({"admin", "services", appname, "check_fiddel"}, call("app_check")).leaf = true
	local coms = require "luci.fiddel.com"
	local com
...
function reset_config()
	luci.sys.call('/etc/init.d/fiddel stop')
	luci.sys.call('[ -f "/usr/share/fiddel/0_default_config" ] && cp -f /usr/share/fiddel/0_default_config /etc/config/fiddel')
...
function link_add_node()
	...
	if chunk and chunk_index ~= nil and total_chunks ~= nil then
		...
		if chunk_index + 1 == total_chunks then
			luci.sys.call("lua /usr/share/fiddel/subscribe.lua add " .. group)
		end
	end
end
...
function gen_client_config()
	...
	luci.sys.call(string.format("/usr/share/fiddel/app.sh run_socks flag=config_%s node=%s bind=127.0.0.1 socks_port=1080 config_file=%s no_run=1", id, id, config_file))
...
function get_redir_log()
	...
	local file_path = "/tmp/etc/fiddel/acl/" .. id .. "/" .. name .. ".log"
...
function get_socks_log()
	local name = http.formvalue("name")
	local path = "/tmp/etc/fiddel/SOCKS_" .. name .. ".log"
...
function get_log()
	-- luci.sys.exec("[ -f /tmp/log/fiddel.log ] && sed '1!G;h;$!d' /tmp/log/fiddel.log > /tmp/log/fiddel_show.log")
	http.write(luci.sys.exec("[ -f '/tmp/log/fiddel.log' ] && cat /tmp/log/fiddel.log"))
end

function clear_log()
	luci.sys.call("echo '' > /tmp/log/fiddel.log")
end

function index_status()
	local e = {}
	e["global_status"] = luci.sys.call("/bin/busybox top -bn1 | grep -v 'grep' | grep '/tmp/etc/fiddel/bin/' | grep 'default' | grep 'global' >/dev/null") == 0
	http_write_json(e)
end
...
function urltest_node()
	...
	local e = {}
	e.index = index
	local result = luci.sys.exec(string.format("/usr/share/fiddel/test.sh url_test_node %s %s", id, "urltest_node"))
	local code = tonumber(luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $1}'") or "0")
	if code ~= 0 then
		local use_time = luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $2}'")
		if use_time:find("%.") then
...
function update_rules()
	...
	luci.sys.call("lua /usr/share/fiddel/rule_update.lua log '" .. update .. "' > /dev/null 2>&1 &")
	http_write_json()
end
...
function server_user_log()
	local id = http.formvalue("id")
	if nixio.fs.access("/tmp/etc/fiddel_server/" .. id .. ".log") then
		local content = luci.sys.exec("cat /tmp/etc/fiddel_server/" .. id .. ".log")
		content = content:gsub("\n", "<br />")
		http.write(content)
	else
		http.write(string.format("<script>alert('%s');window.close();</script>", i18n.translate("Not enabled log")))
...
function server_get_log()
	http.write(luci.sys.exec("[ -f '/tmp/log/fiddel_server.log' ] && cat /tmp/log/fiddel_server.log"))
end

function server_clear_log()
	luci.sys.call("echo '' > /tmp/log/fiddel_server.log")
end
...
local backup_files = {
	"/etc/config/fiddel",
	"/etc/config/fiddel_server",
	"/usr/share/fiddel/domains_excluded"
}

function create_backup()
	local date = os.date("%y%m%d%H%M")
	local tar_file = "/tmp/fiddel-" .. date .. "-backup.tar.gz"
	fs.remove(tar_file)
	local cmd = "tar -czf " .. tar_file .. " " .. table.concat(backup_files, " ")
	api.sys.call(cmd)
	http.header("Content-Disposition", "attachment; filename=fiddel-" .. date .. "-backup.tar.gz")
	http.header("X-Backup-Filename", "fiddel-" .. date .. "-backup.tar.gz")
