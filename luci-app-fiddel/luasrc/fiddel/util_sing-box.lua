module("luci.fiddel.util_sing-box", package.seeall)
...
function gen_config_server(node)
	...
	if node.outbound_node then
		...
		if node.outbound_node == "_iface" and node.outbound_node_iface then
			...
			local outbound_node_t = uci:get_all("fiddel", node.outbound_node)
			...
			outbound = require("luci.fiddel.util_sing-box").gen_outbound(nil, outbound_node_t, "outbound")
		end
		if outbound then
...
function gen_config(var)
	...
	if dns_listen_port then
		...
		if direct_dns_udp_server then
			local nodes_domain = {}
			local nodes_domain_text = sys.exec('uci show fiddel | grep ".address=" | cut -d "\'" -f 2 | grep "[a-zA-Z]$" | sort -u')
