module("luci.fiddel.api", package.seeall)
appname = "fiddel"
local com = require "luci.fiddel.com"
nixio = require "nixio"
fs = require "nixio.fs"
...
LOG_FILE = "/tmp/log/fiddel.log"
CACHE_PATH = "/tmp/etc/fiddel_tmp"
...
function set_cache_var(key, val)
	sys.call(string.format('. /usr/share/fiddel/utils.sh ; set_cache_var %s "%s"', key, val))
end

function get_cache_var(key)
	local val = sys.exec(string.format('. /usr/share/fiddel/utils.sh ; echo -n $(get_cache_var %s)', key))
	if val == "" then val = nil end
	return val
end

function get_new_port()
	local cmd_format = ". /usr/share/fiddel/utils.sh ; echo -n $(get_new_port %s tcp,udp)"
...
local function get_bin_version_cache(file, cmd)
	sys.call("mkdir -p /tmp/etc/fiddel_tmp")
	if fs.access(file) then
		chmod_755(file)
		local md5 = sys.exec("echo -n $(md5sum " .. file .. " | awk '{print $1}')")
		if fs.access("/tmp/etc/fiddel_tmp/" .. md5) then
			return sys.exec("echo -n $(cat /tmp/etc/fiddel_tmp/%s)" % md5)
		else
			local version = sys.exec(string.format("echo -n $(%s %s)", file, cmd))
			if version and version ~= "" then
				sys.call("echo '" .. version .. "' > " .. "/tmp/etc/fiddel_tmp/" .. md5)
				return version
			end
		end
	end
...
function to_move(app_name,file)
	...
	local flag = sys.call('busybox pgrep -af "fiddel/.*'.. app_name ..'" >/dev/null')
	if flag == 0 then
		sys.call("/etc/init.d/fiddel stop")
	end
	...
	sys.call(cmd_rm_tmp)
	if flag == 0 then
		sys.call("/etc/init.d/fiddel restart >/dev/null 2>&1 &")
	end
...
function get_version()
	local version = sys.exec("opkg list-installed luci-app-fiddel 2>/dev/null | awk '{print $3}'")
	if not version or #version == 0 then
		version = sys.exec("apk list luci-app-fiddel 2>/dev/null | awk '/installed/ {print $1}' | cut -d'-' -f4-")
	end
...
function to_check_self()
	...
	local remote_version = sys.exec("echo -n $(grep '^PKG_VERSION' /tmp/fiddel_makefile | head -n 1 | awk -F '=' '{print $2}')")
	exec("/bin/rm", {"-f", tmp_file})
