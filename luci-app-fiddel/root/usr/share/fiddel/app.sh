...
. /usr/share/fiddel/utils.sh
GLOBAL_ACL_PATH=${TMP_ACL_PATH}/default
LUA_UTIL_PATH=/usr/lib/lua/luci/fiddel
	...
	[ -n "$dns_listen_port" ] && {
		...
		[ "${write_ipset_direct}" = "1" ] && {
			...
			if [ "${nftflag}" = "1" ]; then
				...
				local direct_nftset="4#inet#fiddel#${direct_nftset4},6#inet#fiddel#${direct_nftset6}"
			else
...
run_socks() {
	...
	if [ "$type" == "sing-box" ] || [ "$type" == "fiddel" ] || [ "$type" == "xray" ]; then
	...
	# http to socks
	[ -z "$http_flag" ] && [ "$http_port" != "0" ] && [ -n "$http_config_file" ] && [ "$type" != "sing-box" ] && [ "$type" != "fiddel" ] && [ "$type" != "xray" ] && [ "$type" != "socks" ] && {
		json_init
		json_add_string "local_http_port" "${http_port}"
	...
	[ -z "$no_run" ] && [ "${server_host}" != "127.0.0.1" ] && [ "$type" != "sing-box" ] && [ "$type" != "fiddel" ] && [ "$type" != "xray" ] && echo "${node}" >> $TMP_PATH/direct_node_list
}
...
run_global() {
	...
	if [ "${TYPE}" = "xray" ] && [ -n "${XRAY_BIN}" ]; then
		run_func="run_xray"
	elif ([ "${TYPE}" = "sing-box" ] || [ "${TYPE}" = "fiddel" ]) && [ -n "${SINGBOX_BIN}" ]; then
...
start_crontab() {
	...
	for item in $(uci show ${CONFIG} | grep "=nodes" | cut -d '.' -sf 2 | cut -d '=' -sf 1); do
		if [ "$(config_n_get $item type)" = "fiddel" ] && [ "$(config_n_get $item fiddel_auto_update 0)" = "1" ]; then
			fiddel_update_interval=$(config_n_get $item fiddel_update_interval 24)
			remark=$(config_n_get $item remarks)
			local t="0 */${fiddel_update_interval} * * *"
			echo "$t lua $APP_PATH/fiddel.lua update $item > /dev/null 2>&1 &" >>/etc/crontabs/root
			log_i18n 0 "Scheduled tasks: Auto update [%s] Fiddel node (every %s hours)." "${remark}" "${fiddel_update_interval}"
		fi
	done
...
run_ipset_chinadns_ng() {
	...
	[ -n "${nftset}" ] && {
		...
		vps_set_names="inet@fiddel@psw2_vps,inet@fiddel@psw2_vps6"
	}
	cat <<-EOF > $config_file
		bind-addr 127.0.0.1
...
start() {
	busybox pgrep -f /tmp/etc/fiddel/bin > /dev/null 2>&1 && {
