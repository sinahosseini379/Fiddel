...
function copy_instance(var)
	...
	for line in io.lines("/tmp/etc/dnsmasq.conf." .. DEFAULT_DNSMASQ_CFGID) do
		local filter
		if line:find("fiddel") then filter = true end
	...
	if TMP_DNSMASQ_PATH then
		sys.call("rm -rf " .. TMP_DNSMASQ_PATH .. "/*fiddel*")
	end
...
function add_rule(var)
	...
	local cache_text = ""
	local nodes_address_md5 = sys.exec("echo -n $(uci show fiddel | grep '\\.address') | md5sum")
	...
	local setflag_4= (NFTFLAG == "1") and "4#inet#fiddel#" or ""
	local setflag_6= (NFTFLAG == "1") and "6#inet#fiddel#" or ""
